...
st.markdown(
    ...
    /* Kotak login kecil & terpusat */
    .st-key-login_box {
        background: #FFFFFF;
        border: 1px solid var(--so-border);
        border-radius: 18px;
        padding: 1.4rem 1.35rem 1.1rem 1.35rem;
        box-shadow: 0 10px 28px rgba(31,73,125,0.14);
    }
    .st-key-login_box [data-testid="stForm"] {
        border: none;
        padding: 0;
    }
    .so-hero-title {
    ...
    /* ========================================================
       HALAMAN LOGIN - kartu putih minimalis (ala referensi)
       ======================================================== */
    .login-brand-row {
        display: flex;
        align-items: center;
    ...
    .st-key-login_box input[type="text"],
    .st-key-login_box input[type="password"] {
        border-radius: 10px !important;
        border: 1px solid #E3E8EF !important;
        padding: 0.7rem 0.9rem !important;
        background: #FFFFFF !important;
    }
    .st-key-login_box [data-testid="stTextInput"] {
        margin-bottom: .5rem;
    }
    .st-key-login_box div.stButton > button[kind="primary"],
    .st-key-login_box div.stFormSubmitButton > button[kind="primary"] {
    ...
    .st-key-login_box div.stButton > button[kind="primary"]:hover,
    .st-key-login_box div.stFormSubmitButton > button[kind="primary"]:hover {
        background: #2563EB !important;
    }
    .st-key-login_box .link-btn button {
    ...
    .st-key-login_box .link-btn-right button {
        float: right;
    }
    .login-footer-text {
        text-align: center;
...
def _render_app_header():
    """Header biru 'Stock Opname Pro' -- dipakai di halaman
    utama setelah login, maupun di atas kartu halaman login."""
...
def prepare_data(df):
    ...
    # Baris tanpa Barcode bukan baris data SO yang valid
    # (mis. baris subtotal ikutan dari file sumber) --
    # diabaikan sepenuhnya, tidak dihitung ke mana pun.
    df = df[
        df["Barcode"] != ""
    ].reset_index(drop=True)
...
def apply_number_format(
    ...
    Nilai negatif otomatis ditampilkan warna merah -- ini
    ditulis lewat kode warna di number_format ([Red] pada
    bagian negatif), bukan lewat font statis. Jadi warnanya
    "hidup": kalau selnya berisi rumus dan Qty Fisik/Qty POS
    diedit manual di Excel sampai hasilnya berubah tanda,
    Excel otomatis mewarnai ulang (merah/hitam) tanpa perlu
    dibuka & disimpan ulang lewat Python.

    Parameter `value` sudah tidak dipakai untuk menentukan
    warna (Excel yang menentukan sendiri dari hasil rumus),
    tapi tetap diterima supaya pemanggil lama tidak perlu
    diubah.
    """
...
def create_final_excel(
    ...
    metadata_fields = [
        ...
        (
            "Tanggal SO",
            format_tanggal_short(metadata["Date"]),
        ),
        ...
        for _, source in part.iterrows():
            ...
            for col_idx, column in enumerate(
                ...
                if column == "Selisih":
                    ...
                    column == "Remark"
                    and category != "TERTUKAR"
                ):
                    # Ikuti tanda Selisih secara live: kalau
                    # Qty Fisik/Qty POS diedit manual di Excel,
                    # status Remark otomatis ikut berubah.
                    # Khusus TERTUKAR dibiarkan teks statis
                    # karena statusnya bukan ditentukan dari
                    # tanda Selisih baris itu sendiri.
                    selisih_ref = (
                        f"{col_letter['Selisih']}"
                        f"{current_row}"
                    )
                    formula = (
                        f'=IF({selisih_ref}<0,"Minus",'
                        f'IF({selisih_ref}>0,"Plus","Klop"))'
                    )
    ...
    if include_hm:
        def apply_rupiah_format(cell):
            ...
            -Rp26.404 / Rp0 / Rp500.000, merah otomatis untuk
            nilai negatif (kode warna di number_format, jadi
            tetap ikut berubah walau sel berisi rumus dan
            hasilnya diedit ulang di Excel).
            """
            cell.number_format = (
                '"Rp"#,##0;[Red]-"Rp"#,##0;"Rp"0'
            )
    ...
    # Total HM, Remark) diestimasi dari nilai asli di
    # dataframe.
    formula_columns = {
        col_letter["Selisih"]: (
            df["Selisih"],
...
def format_tanggal_short(value):
    """
    Ubah tanggal apa pun dari file (2026-09-08, 08/09/2026,
    8-Sep-2026, dst) jadi format singkat "08 Sep 2026"
    (2 digit hari, 3 huruf awal nama bulan, 4 digit tahun).
    Kalau tidak bisa dibaca, dikembalikan apa adanya.
    """
...
#
# Alur akses:
#   - Anda (admin) punya username & password sendiri, dibuat
#     otomatis saat pertama kali dijalankan (lihat
#     DEFAULT_ADMIN_USERNAME / DEFAULT_ADMIN_PASSWORD di bawah).
#     GANTI password default ini lewat menu "Ganti Password
#     Admin" begitu aplikasi pertama kali dibuka.
#   - User lain mendaftar sendiri lewat tab "Daftar Akun Baru"
#     -> status akunnya "pending" (menunggu).
#   - Akun baru baru bisa dipakai login setelah Anda setujui
...
def _generate_temp_password():
    """Password sementara acak, mudah dibaca (tanpa 0/O/1/l)."""

    alphabet = "23456789ABCDEFGHJKMNPQRSTUVWXYZ"

    return "".join(
        pysecrets.choice(alphabet) for _ in range(8)
    )


def _hash_password(password, salt_hex=None):
    """Hash password dengan PBKDF2-SHA256 + salt acak."""
...
def _submit_registration(
    ...
    if not re.match(r"^[a-z0-9._-]+$", username):
        return False, (
            "Username hanya boleh huruf kecil, angka, titik, "
            "underscore, atau strip (tanpa spasi)."
        )

    if username == db["admin"]["username"].strip().lower():
        return False, "Username ini sudah dipakai."

    existing = db["users"].get(username)

    if existing:
        if existing["status"] == "pending":
            return False, (
                "Kamu sudah pernah mengajukan akses dengan "
                "username ini. Tunggu persetujuan admin."
            )
        if existing["status"] == "approved":
            return False, (
                "Username ini sudah terdaftar dan aktif. "
                "Silakan login."
            )

    salt_hex, digest = _hash_password(password)
    ...
    if security_question and security_answer and security_answer.strip():
        ...
        sec_salt, sec_hash = _hash_password(
    ...
    return True, (
        "Permintaan akses terkirim. Silakan tunggu persetujuan "
        "admin sebelum bisa login."
    )


def _authenticate(db, username, password):
    """Return (ok, role, display_name, error_message)."""

    username = username.strip().lower()

    admin = db["admin"]

    if username == admin["username"].strip().lower():
        if _verify_password(password, admin["salt"], admin["hash"]):
            return True, "admin", "Admin", ""
        return False, None, None, "Password admin salah."

    record = db["users"].get(username)

    if not record:
        return False, None, None, "Username tidak ditemukan."
    ...
    if status == "rejected":
        return False, None, None, (
            "Permintaan akses kamu ditolak admin. Hubungi "
            "admin untuk info lebih lanjut."
        )

    if status == "disabled":
        return False, None, None, (
            "Akun kamu dinonaktifkan admin. Hubungi admin "
            "untuk info lebih lanjut."
        )

    if not _verify_password(password, record["salt"], record["hash"]):
        return False, None, None, "Password salah."

    return True, "user", record["display_name"], ""


def _get_account_and_question(db, username):
    """
    Return (kind, record, question) untuk username yang login-
    eligible (admin atau user berstatus approved).
    ...
    if username == db["admin"]["username"].strip().lower():
        return "admin", db["admin"], db["admin"].get(
            "security_question"
        )
    ...
    if record and record["status"] == "approved":
        return "user", record, record.get("security_question")

    return None, None, None


def _reset_password_with_answer(
    db, username, answer, new_password
):

    kind, record, question = _get_account_and_question(
        db, username
    )

    if kind is None:
        return False, (
            "Akun tidak ditemukan atau belum aktif. Hubungi "
            "admin."
        )

    if not question:
        return False, (
            "Akun ini belum mengatur pertanyaan keamanan. "
            "Minta admin untuk reset password kamu."
        )
    ...
    if not _verify_password(
        answer.strip().lower(),
        record["security_salt"],
        record["security_hash"],
    ):
        return False, "Jawaban keamanan tidak sesuai."

    salt_hex, digest = _hash_password(new_password)
    ...
    return True, (
        "Password berhasil diganti. Silakan login dengan "
        "password baru."
    )


def _set_security_question(
    db, kind, record, question, answer
):

    salt_hex, digest = _hash_password(answer.strip().lower())

    record["security_question"] = question
    record["security_salt"] = salt_hex
    record["security_hash"] = digest
...
def _render_login_view(db):

    st.markdown(
        '<div class="login-eyebrow">Silakan lengkapi data Anda</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    forgot_col_l, forgot_col_mid, forgot_col_r = st.columns(
        [1, 1.4, 1]
    )
    with forgot_col_mid:
        if st.button(
    ...
    if st.button(
        ...
        ok, role, display_name, err = _authenticate(
    ...
    register_col_l, register_col_mid, register_col_r = st.columns(
        [1, 1.4, 1]
    )
    with register_col_mid:
        if st.button(
            "Daftar di sini",
            key="goto_register",
            use_container_width=True,
...
def _render_register_view(db):

    st.markdown(
        '<div class="login-eyebrow">Ajukan akses ke aplikasi</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    st.caption(
        "Admin perlu menyetujui dulu sebelum kamu bisa login."
    )
    ...
    with st.form("register_form"):
        ...
        reg_username = st.text_input(
            "Username",
            help=(
                "Huruf kecil, angka, titik, underscore, "
                "atau strip. Tanpa spasi."
            ),
        )
        ...
        reg_question = st.selectbox(
            ...
            SECURITY_QUESTIONS,
            help=(
                "Dipakai untuk reset password sendiri kalau "
                "lupa, tanpa perlu menunggu admin."
            ),
        )
        reg_answer = st.text_input("Jawaban Keamanan")
        reg_note = st.text_input(
            "Toko / Keterangan (opsional)",
            placeholder="contoh: Toko HPN",
        )
    ...
    if reg_submitted:
        ...
        if reg_password != reg_password2:
            ...
            st.error("Jawaban keamanan wajib diisi.")
        else:
            ok, msg = _submit_registration(
                db,
                reg_username,
                reg_name,
                reg_password,
    ...
    if st.button("← Kembali ke Login", key="goto_login_1"):
        _goto_view("login")
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)


def _render_forgot_view(db):

    st.markdown(
        '<div class="login-eyebrow">Reset password akun kamu</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
    ...
    st.caption(
        "Masukkan username untuk melihat pertanyaan "
        "keamanan kamu."
    )

    forgot_username = st.text_input(
        "Username", key="forgot_username_input"
    )

    if st.button(
        "Cek Akun",
        key="forgot_check_btn",
        type="primary",
        use_container_width=True,
    ):
        ...
        if kind is None:
            st.error(
                "Akun tidak ditemukan atau belum disetujui "
                "admin."
            )
            st.session_state.pop("forgot_question", None)
        elif not question:
            st.warning(
                "Akun ini belum mengatur pertanyaan "
                "keamanan. Minta admin untuk reset password "
                "kamu."
            )
            st.session_state.pop("forgot_question", None)
        else:
            st.session_state.forgot_question = question
            st.session_state.forgot_username_confirmed = (
                forgot_username.strip().lower()
            )

    if st.session_state.get("forgot_question"):

        st.info(st.session_state.forgot_question)

        with st.form("forgot_reset_form"):

            forgot_answer = st.text_input("Jawaban Kamu")
            forgot_new_pwd = st.text_input(
                "Password Baru", type="password"
            )
            forgot_new_pwd2 = st.text_input(
        ...
        if forgot_submitted:
            ...
            if forgot_new_pwd != forgot_new_pwd2:
                st.error("Konfirmasi password tidak sama.")
            else:
                ok, msg = _reset_password_with_answer(
                    db,
                    st.session_state[
                        "forgot_username_confirmed"
                    ],
                    forgot_answer,
                    forgot_new_pwd,
                )
    ...
    if st.button("← Kembali ke Login", key="goto_login_2"):
        st.session_state.pop("forgot_question", None)
        _goto_view("login")
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
...
def _render_login_page():
    ...
    with col_c:
        ...
        try:
            login_box = st.container(key="login_box")
        except TypeError:
            # Streamlit versi lama tanpa parameter key -- tetap
            # jalan, hanya saja tampilan kotaknya polos.
        ...
        with login_box:
            ...
            if view == "register":
                _render_register_view(db)
            elif view == "forgot":
                _render_forgot_view(db)
            else:
                _render_login_view(db)


def _render_admin_panel():
    """Panel kelola akses user, ditaruh di sidebar (khusus admin)."""
    ...
    other = {
        u: r
        for u, r in db["users"].items()
        if r["status"] in ("rejected", "disabled")
    }
    ...
    with st.expander(
        ...
        with tab_pending:
            ...
            if not pending:
                st.caption("Tidak ada permintaan menunggu.")
            ...
            for uname, rec in pending.items():
                ...
                with c1:
                    if st.button(
                        "✅ Setujui",
                        key=f"approve_{uname}",
                        use_container_width=True,
                    ):
                        db = _load_auth_db()
                        db["users"][uname]["status"] = "approved"
                        db["users"][uname]["decided_at"] = (
                            _now_text()
                        )
                        _save_auth_db(db)
                        st.rerun()
                ...
                with c2:
                    if st.button(
                        ...
                        db = _load_auth_db()
                        db["users"][uname]["status"] = "rejected"
                        db["users"][uname]["decided_at"] = (
                            _now_text()
                        )
                        _save_auth_db(db)
        ...
        with tab_active:
            ...
            st.caption(
                "Tambah user aktif secara langsung (tanpa "
                "lewat pendaftaran):"
            )
            ...
            with st.form("admin_add_user_form"):
                ...
                add_name = st.text_input(
                    "Nama Lengkap", key="add_name"
                )
                add_username = st.text_input(
                    "Username", key="add_username"
                )
                add_password = st.text_input(
                    "Password", type="password", key="add_pwd"
                )
            ...
            if add_submitted:
                ...
                if ok:
                    uname_new = add_username.strip().lower()
                    db["users"][uname_new]["status"] = "approved"
                    db["users"][uname_new]["decided_at"] = (
                        _now_text()
                    )
                    _save_auth_db(db)
                    st.success(
                        f"User '{uname_new}' ditambahkan & aktif."
                    )
                    st.rerun()
                else:
                    st.error(msg)
            ...
            if not approved:
                st.caption("Belum ada user aktif.")

            for uname, rec in approved.items():

                st.markdown(
                    f"**{rec['display_name']}** (`{uname}`)"
                )
                ...
                with c1:
                    if st.button(
                        ...
                        db = _load_auth_db()
                        db["users"][uname]["salt"] = salt_hex
                        db["users"][uname]["hash"] = digest
                        _save_auth_db(db)
                        st.session_state[
                            f"temp_pwd_{uname}"
                        ] = temp_pwd
                ...
                with c2:
                    if st.button(
                        "🚫 Nonaktifkan",
                        key=f"disable_{uname}",
                        use_container_width=True,
                    ):
                        db = _load_auth_db()
                        db["users"][uname]["status"] = (
                            "disabled"
                        )
                        db["users"][uname]["decided_at"] = (
                            _now_text()
                ...
                if st.session_state.get(f"temp_pwd_{uname}"):
                    st.success(
                        "Password baru untuk "
                        f"`{uname}`: "
                        f"**{st.session_state[f'temp_pwd_{uname}']}** "
                        "— sampaikan ke user, minta ganti "
                        "lagi setelah login."
                    )
                    if st.button(
                        "Sudah dicatat, sembunyikan",
                        key=f"hide_temp_{uname}",
                    ):
        ...
        with tab_other:
            ...
            if not other:
                st.caption(
                    "Tidak ada user yang ditolak/nonaktif."
                )

            for uname, rec in other.items():
        ...
        st.caption(
            "Untuk ganti password/pertanyaan keamanan akun "
            "admin kamu sendiri, buka menu 🔑 Ganti Password "
            "& Keamanan di sidebar."
        )

        st.download_button(
            "⬇️ Unduh Backup Data User (JSON)",
            data=json.dumps(
                db, ensure_ascii=False, indent=2
            ),
            file_name="auth_users_backup.json",
...
def _render_change_password_self():
    """
    Menu "Ganti Password & Keamanan" untuk siapa pun yang
    sedang login (admin maupun user biasa), mengubah data
    akun miliknya sendiri.
    """
    ...
    if st.session_state.auth_role == "admin":
        kind, record = "admin", db["admin"]
    else:
        kind, record = "user", db["users"][username]

    with st.expander("🔑 Ganti Password & Keamanan"):

        with st.form("self_change_pwd_form"):

            cur_pwd = st.text_input(
                "Password Saat Ini", type="password"
            )
        ...
        if pwd_submitted:
            ...
            record = (
                db["admin"]
                if kind == "admin"
                else db["users"][username]
            )

            if not _verify_password(
                cur_pwd, record["salt"], record["hash"]
            ):
                ...
                salt_hex, digest = _hash_password(new_pwd)
                record["salt"] = salt_hex
                record["hash"] = digest
                _save_auth_db(db)
        ...
        current_q = record.get("security_question")

        st.caption(
            "Pertanyaan keamanan (dipakai untuk reset "
            "password sendiri kalau lupa). "
            + (
                f"Saat ini: _{current_q}_"
                if current_q
                else "Belum diatur."
            )
        )

        with st.form("self_security_form"):

            sec_cur_pwd = st.text_input(
                "Password Saat Ini",
                type="password",
                key="sec_cur_pwd",
            )
            sec_question = st.selectbox(
                "Pertanyaan Keamanan",
                SECURITY_QUESTIONS,
                key="sec_question_select",
            )
            sec_answer = st.text_input(
                "Jawaban Keamanan", key="sec_answer_input"
            )
            sec_submitted = st.form_submit_button(
        ...
        if sec_submitted:
            ...
            if not _verify_password(
                sec_cur_pwd, record["salt"], record["hash"]
            ):
                st.error("Password saat ini salah.")
            elif not sec_answer.strip():
                st.error("Jawaban keamanan wajib diisi.")
            else:
                _set_security_question(
                    db, kind, record, sec_question, sec_answer
                )
                st.success(
...
with st.sidebar:
    ...
    st.markdown(
        f"👤 **{st.session_state.auth_display_name}** "
    ...
    show_more_menu = st.checkbox(
        "⚙️ Setting",
        value=False,
        key="show_more_sidebar_menu",
        help=(
            "Tampilkan menu Ganti Password & Keamanan"
            + (
                " dan Kelola Akses User."
                if st.session_state.auth_role == "admin"
                else "."
            )
        ),
    ...
    if show_more_menu:
        _render_change_password_self()

    st.markdown("---")

if (
    st.session_state.auth_role == "admin"
    and st.session_state.get("show_more_sidebar_menu")
):
    with st.sidebar:
        _render_admin_panel()
        st.markdown("---")
...
st.caption(
    "Unggah file Data mentah Stock Opname (.xlsx / .csv) "
    "untuk melakukan parsing, kategorisasi otomatis, dan export laporan terstruktur."
)
...
with st.sidebar:
    ...
    with st.expander(
        ...
        ba_mengetahui = st.text_input(
            ...
            value="",
            placeholder="Asenk, Bpk. Jeffry, Risviana, Bpk. Yudi",
        )

        ba_menyetujui = st.text_input(
            "Menyetujui (pisahkan koma)",
            value="",
            placeholder="Bpk. Andi. S",
        )
...
try:
    ...
    _std_df_raw = map_standard_columns(
        df_raw
    )

    hm_missing = (
        "HM" not in _std_df_raw.columns
    )

    rows_skipped_no_barcode = 0

    if "Barcode" in _std_df_raw.columns:
        _barcode_check = (
            _std_df_raw["Barcode"]
            .fillna("")
            .astype(str)
    ...
    if rows_skipped_no_barcode > 0:
        st.info(
            f"ℹ️ {rows_skipped_no_barcode} baris tanpa "
            "**Barcode** diabaikan (tidak ikut dihitung), "
            "kemungkinan baris subtotal ikutan dari file "
            "sumber."
        )
...
metadata_line = (
    ...
    f"Tanggal SO {format_tanggal_short(metadata['Date']) or '-'} | "
